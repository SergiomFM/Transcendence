...
export namespace Events
{
	...
	function playerDashEvent(key: any, pong: Pong)
	{
		...
		{
			if (pong.player2.dashReady)
				pong.player2.dashActive = true;
		}
	}
	...
	export function registerEvents(pong: Pong)
	{
		...
		pong.scene.actionManager.registerAction(
			new ExecuteCodeAction(ActionManager.OnKeyDownTrigger, (event) => keyPressEvent(pong, pong.camera, event)));

		// Registering Key Releasing
		pong.scene.actionManager.registerAction(
			new ExecuteCodeAction(ActionManager.OnKeyUpTrigger, (event) => keyReleaseEvent(pong, event)));
	}
