import { SphereParticleEmitter, Color4, Tools, Scalar, ParticleSystem, HemisphericParticleEmitter, Vector3, Mesh } from "@babylonjs/core";
//import { createSpell } from "./pongVFX";
import { Pong, Player } from "./pong";
import { createSpellParticles, splashEffect, updateArena, COLOR_DIFFERENCE, OFFENSIVE_SPELL_VFX, COUNTER_SPELL_VFX, OFFENSIVE_CAST_VFX, COUNTER_CAST_VFX, diskExplosion } from "./pongVFX";
import { useSpellAnimation, OFFENSIVE_CASTING, COUNTER_CASTING } from "./pongAnimations";
...
export abstract class Spell
{
	...
	constructor(pong: Pong, player: Player, name: string)
	{
		...
		{
			this.particle = createSpellParticles(this.pong.scene, this, OFFENSIVE_SPELL_VFX);
		...
		{
			this.particle = createSpellParticles(this.pong.scene, this, COUNTER_SPELL_VFX);
			this.castingAnimation = COUNTER_CASTING;
			this.castingVFX = COUNTER_CAST_VFX;
			this.hand = player.leftHand;
			this.maxSize = 0.05;
		}
		this.hemisphericEmitter = this.particle.particleEmitterType as HemisphericParticleEmitter

		// Figuring out the spell effects direction
		if (name.includes("Player"))
			this.castingAngle = Math.PI * 0.5;
		else
			this.castingAngle = -Math.PI * 0.5;
	...
	resetSpell()
	{
		...
		this.active = false;
		this.cooldownElapsed = 0;
		(this.particle.particleEmitterType as SphereParticleEmitter).radius = this.initialSize;
	}
	...
	activateSpell()
	{
		...
		setTimeout(() => {
			splashEffect(this.pong.scene, this.hand.absolutePosition, 1, this.castingAngle, this.castingVFX);
	...
	spellLoop(delta: number)
	{
		...
		if (!this.ready)
		{
			...
			this.hemisphericEmitter.radius = this.initialSize + (this.maxSize - this.initialSize) * t;
...
export class BallAngleSwitch extends Spell
{
	...
	loopAddon(elapsedTime: number) { elapsedTime; }
}
...
class BallShot extends Spell
{
	...
	useSpell()
	{
		...
		if (this.pong.ball.angle > 0 && this.pong.ball.angle < Math.PI)
			...
			this.pong.ball.setAngle(Tools.ToRadians(270));
	}
...
class BallPortal extends Spell
{
	...
	{		
		...
		if (this.lastZDir == Math.sign(this.pong.ball.sin)
			&& this.lastXDir != Math.sign(this.pong.ball.cos))
		{
			this.pong.ball.x *= -1;
			this.pong.ball.setAngle(Math.PI - this.pong.ball.angle);
...
export class BallStop extends Spell
{
	...
	loopAddon(elapsedTime: number)
	{
		this.pong.ball.set(this.originalPosition.x, this.originalPosition.y, this.originalPosition.z);
...
class BallIman extends Spell
{
	...
	loopAddon(elapsedTime: number)
	{
		...
		let ballToPaddleAngle = Math.atan2(this.player.z - this.pong.ball.z, this.player.x - this.pong.ball.x);
		
		let direction = -1;
		if (this.pong.ball.angle - ballToPaddleAngle < ballToPaddleAngle - this.pong.ball.angle)
			direction = 1;
