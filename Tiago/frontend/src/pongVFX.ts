import { Scene, Vector3, Scalar, ParticleSystem, Color4, DynamicTexture, Matrix, BoxParticleEmitter, HemisphericParticleEmitter } from "@babylonjs/core";
...
export function createWallParticles(scene: Scene, begin: Vector3, end: Vector3, wallNum: number)
{
	const particle = createBaseParticle(scene, WALL_VFX);
	particle.id += wallNum;

	// Creating a plane to emit the particles upwards
	particle.createBoxEmitter(new Vector3(0, 0, 0),
...
export function splashEffect(scene: Scene, position: Vector3, splashPower: number, angle: number, VFX: Object)
{
	...
	cone.direction1 = new Vector3(Math.cos(spread), -Math.sin(spread), -Math.sin(spread)).normalize();
	cone.direction2 = new Vector3(Math.cos(spread), Math.sin(spread), Math.sin(spread)).normalize();
...
function wallExplosion(scene: Scene, wallName: string)
{
	...
	particle.particleEmitterType = scene.getParticleSystemById(wallName).particleEmitterType;
...
export function createSpellParticles(scene: Scene, spell: Spell, VFX: object)
{
	...
	// Creating a Sphere to emit the particles
	let hemisphericEmitter = new HemisphericParticleEmitter(spell.initialSize, 0, 1);

	// Called for each particle to set its initial velocity
	const originalPositionFunction = hemisphericEmitter.startPositionFunction.bind(hemisphericEmitter);
	hemisphericEmitter.startPositionFunction = function (worldMatrix, position, particle, isLocal)
	{
	...
	// Called for each particle to set its initial velocity
	const originalDirectionFunction = hemisphericEmitter.startDirectionFunction.bind(hemisphericEmitter);
	hemisphericEmitter.startDirectionFunction = function (worldMatrix, direction, particle, isLocal)
	{
		originalDirectionFunction(worldMatrix, direction, particle, isLocal);

		// Flip half the particles downward
		if (Math.random() < 0.5)
			direction.y *= -1; // invert Y to go down
	};
